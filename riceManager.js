...
const ConfirmIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let speechOutput = `ご飯は残り${counter}個です。`;
...
const RegisterIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let attributes = {"counter":amount};

        attributesManager.setPersistentAttributes(attributes);

        await attributesManager.savePersistentAttributes();

        let speechOutput = `${attributes.counter}個で登録しました。`;
...
const ReduceIntentHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'ReduceIntent';
    },
    async handle(handlerInput) {
        const attributesManager = handlerInput.attributesManager;
        const attributes = await attributesManager.getPersistentAttributes() || {};
        ...
        if (counter === 0){
            const speakOutput = "ご飯はすでにゼロ個です。何個で登録しますか？"
            return handlerInput.responseBuilder
            .speak(speakOutput)
            .reprompt(speakOutput)
            .getResponse();
